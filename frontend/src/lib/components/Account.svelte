<script lang="ts">
	...
	// Add country badge if country prop is not null
	if (country != null) {
...
<div class="{compact ? '' : 'card card-hover p-4'} flex flex-row gap-3">
	<!-- Avatar component with different sizes deopending on compact prop -->
	<Avatar
		src={avatar}
	...
	<div>
		<!-- Row for username and badges -->
		<div class="flex flex-row gap-1">
			<h1>{username}</h1>
			<!-- Loop through badges and dispay each badge with popup and hover effect -->
		...
		<!-- Display the about me section if not in compact mode -->
		{#if !compact}
			<p>{about_me}</p>
		{/if}
	</div>
